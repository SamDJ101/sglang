...
def _runner(*, has_marker: bool = False, breakable: bool = False):
    ...
    runner.backend = (
        create_autospec(BreakableCudaGraphBackend, instance=True) if breakable else None
    )
    return runner
...
@pytest.mark.parametrize(
    "declared, has_marker, breakable, expected",
    [
        # The backend's declaration decides where the record lands.
        (SharedReadEnds.IN_REPLAY, True, False, SharedReadEnds.IN_REPLAY),
        # Nowhere to record in-graph -> fall back to the pre-replay record.
        (SharedReadEnds.IN_REPLAY, False, False, SharedReadEnds.PRE_REPLAY),
        # Breakable replay rereads shared state between segments, after the
        # in-graph marker -> demote to the post-replay record.
        (SharedReadEnds.IN_REPLAY, True, True, SharedReadEnds.POST_REPLAY),
        # Only an in-graph declaration is demoted; the rest pass through.
        (SharedReadEnds.POST_REPLAY, False, False, SharedReadEnds.POST_REPLAY),
    ],
)
def test_resolve_shared_read_ends(declared, has_marker, breakable, expected):
    runner = _runner(has_marker=has_marker, breakable=breakable)
