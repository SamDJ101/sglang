...
//! `/server_info` for workers that predate the field on `/model_info`. The
//! two agree on any worker that has not swapped weights.
//!
//! # Failure semantics
//!
...
/// PD classification derived from a worker's `/server_info` response — the
/// launch record, which is where this belongs: a disaggregation role is fixed
/// at launch and no request moves it.
///
...
mod tests {
    ...
    /// A worker that serves `/server_info` only — the shape of an SGLang that
    /// predates `served_model_name` on `/model_info`, so every test built on
    /// this helper also exercises the fallback.
    ...
    /// The identity comes from `/model_info`, which is the surface a weight
    /// update moves; `/server_info` still answers with the launch record.
    #[tokio::test]
    ...
    /// `/model_info` present but without the field (an older SGLang) falls back
    /// to `/server_info` rather than registering an empty `model_ids`.
    #[tokio::test]
    ...
    /// A worker whose `/server_info` is unreachable still registers under the
    /// name `/model_info` answered with: the two requests fail independently.
    #[tokio::test]
